...
| `CAPSULE_PRICE` | запасная цена капсулы, когда прайса не добыть | `20000` |
...
lib/
  ...
  core/
    ...
    pricing/      цена капсулы: прайс сервера с откатом на значение сборки
...
- цену отдаёт только админский `/admin/prices/current`; `CapsulePrice`
  спрашивает его при любой роли и молча откатывается на `CAPSULE_PRICE` из
  сборки, когда сервер отвечает отказом (водителю — 403). Админские экраны
  считают по живому прайсу, водительские — по значению сборки, пока цена не
  появится и у водителя.
